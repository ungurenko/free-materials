...
import { projects, howToUseSteps, type Project } from "@/content/leadmagnet";
...
export default function ProjectGallery() {
  ...
  const handleCopy = async (project: Project) => {
    ...
    setCopyStatus(copied ? "Промпт скопирован" : "Не удалось скопировать промпт");
      ...
      <div className="project-grid mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => (
          <button
            ...
            aria-label={`Открыть проект: ${project.cardTitle}`}
          ...
          >
            ...
            <span className="flex flex-1 flex-col px-2 pb-2 pt-5">
              ...
              <span className="mt-4 flex flex-wrap gap-2">
                <span className="pill border border-lime-300 bg-lime-100 px-2.5 py-1.5 text-[9.5px] text-lime-700">Уровень: {project.level}</span>
                <span className="pill border border-line bg-milk px-2.5 py-1.5 text-[9.5px] text-ink-soft">{project.time}</span>
              </span>
              <span className="open-hint mt-4 text-sm font-semibold text-lime-700 underline decoration-lime-300 underline-offset-4">Открыть промпт</span>
            </span>
          </button>
        ))}
      </div>
        ...
        >
          ...
          >
            <div className="sticky top-0 z-10 flex justify-end border-b border-line bg-paper/90 px-4 py-3 backdrop-blur-xl">
              <button ref={closeButton} type="button" onClick={() => closeProject()} aria-label="Закрыть окно" className="grid size-10 place-items-center rounded-full border border-line bg-milk text-2xl leading-none text-ink transition-colors hover:border-lime-500 hover:bg-lime-100">×</button>
            </div>
            ...
            <div className="px-5 pb-7 pt-5 sm:px-8 sm:pb-9">
              ...
              <section className="mt-8 border-t border-line pt-6">
                <h3 className="font-display text-base font-semibold text-ink">Подходящие сервисы</h3>
              ...
              {activeProject.examples.length > 0 && (
                <section className="mt-7 border-t border-line pt-6">
                  <h3 className="font-display text-base font-semibold text-ink">Примеры применения</h3>
              ...
              {activeProject.replace && (
                <section className="mt-7 border-t border-line pt-6">
                  <h3 className="font-display text-base font-semibold text-ink">Что заменить</h3>
              ...
              <section className="mt-7 border-t border-line pt-6">
                <h3 className="font-display text-base font-semibold text-ink">Готовый промпт</h3>
                <div className="mt-3 overflow-hidden rounded-2xl border border-line bg-[#f1f2e9]">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-paper/80 px-4 py-3">
                    <span className="font-mono text-xs uppercase tracking-[0.1em] text-ink-faint">Промпт</span>
                    <button type="button" onClick={() => handleCopy(activeProject)} className="btn-primary h-10 px-4 text-sm">
                      {copyStatus === "Промпт скопирован" ? <IconCheck className="size-4" /> : <IconCopy className="size-4" />}
                      Скопировать промпт
                    </button>
                  </div>
              ...
              <section className="mt-7 border-t border-line pt-6">
                <h3 className="font-display text-base font-semibold text-ink">Как использовать</h3>
